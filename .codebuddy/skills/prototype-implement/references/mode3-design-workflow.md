...
```
⛔ STOP - 在 Mode 3 下，禁止跳过设计系统生成！

没有 URL / 没有截图 = 必须走 Mode 3
Mode 3 = 必须先执行 Phase 0: Design Thinking (frontend-design skill) 获取创意指导
不能跳过设计系统生成直接写代码！

❌ 错误: 用户需求 → 直接生成 React 代码
✅ 正确: 用户需求 → frontend-design → design_system.json → React 代码
```
...
## Phase 3: Quality Review (Optional)
