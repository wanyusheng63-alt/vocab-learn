...
**Generate `docs/product/features.md` before writing any code** (Mode 3 only — not needed for URL clone or screenshot modes).

**Validation**: If Mode 3 and `docs/product/features.md` does not exist, STOP and create it first.
...
> **🚨 Template uses Tailwind v4, syntax differs significantly from v3!**
>
> **Using v3 syntax will cause styles to not apply at all!**

**⛔ v4 Deprecated/Renamed Classes**:

| ❌ v3 (Don't use) | ✅ v4 Replacement | Note |
|--------------|----------|------|
| `flex-grow` | `grow` | Renamed |
| `flex-shrink` | `shrink` | Renamed |
| `overflow-ellipsis` | `text-ellipsis` | Renamed |
| `shadow-sm` | `shadow-xs` | Shadow scale adjusted |
| `rounded-sm` | `rounded-xs` | Border radius scale adjusted |
| `blur-sm` | `blur-xs` | Blur scale adjusted |
| `outline-none` | `outline-hidden` | Clearer semantics |
| `bg-opacity-50` | `bg-black/50` | Use color modifiers |
| `text-opacity-50` | `text-black/50` | Use color modifiers |

**⛔ v4 Default Value Changes (Common Pitfalls)**:

| Class | v3 Default | v4 Default | Migration |
|-----|--------|--------|---------|
| `ring` | 3px, blue-500 | 1px, currentColor | Use `ring-3 ring-blue-500` |
| `border` | gray-200 | currentColor | Explicitly specify `border-gray-200` |
| `outline` | No width | 1px | No need to add `outline-1` |

**⛔ v4 Syntax Changes**:

```tsx
// ❌ Important modifier position (v3)
<div className="!flex !bg-red-500">
// ✅ v4: Exclamation mark goes after
<div className="flex! bg-red-500!">

// ❌ CSS variable reference (v3)
<div className="bg-[--brand-color]">
// ✅ v4: Use parentheses
<div className="bg-(--brand-color)">

// ❌ Grid comma separator (v3)
...
1. Do not use deprecated/renamed class names
2. Explicitly specify color and width for `ring`/`border`
3. Important modifier goes after class name (`flex!` not `!flex`)

### ⛔⛔⛔ Web Content Extraction (Mode 1 CRITICAL)

> **MUST use `web_content_fetcher.py`, WebFetch tool is FORBIDDEN!**

```bash
# ✅ CORRECT
python3 scripts/web_content_fetcher.py <url>

# ❌ FORBIDDEN - WebFetch tool cannot handle SPA/dynamic pages
```

### ⛔⛔⛔ Unicode Emoji Prohibition (CRITICAL - Zero Tolerance)

> **🚨 This is a HARD RULE, violations are serious errors! This rule has been repeatedly violated, must pay special attention!**
>
> **Generated code MUST NOT contain any Unicode emoji characters (e.g., 🚀 ✨ 📧 🎨 etc.)**
>
> **Violating this rule = Task failure, must regenerate!**

**Unicode emoji degrades professional and premium feel, MUST use lucide-react icons or SVG instead:**
...
```tsx
// ❌ FORBIDDEN - Unicode emoji (Generating such code = Task failure)
...
1. Search code for Unicode emoji characters
2. If found, MUST replace with lucide-react icons
3. **不自查就提交 = 违反规则**
...
📖 **Creative Animations**: Use **ui-animation** skill for ReactBits creative effects (Hero text, backgrounds, cursors)
...
Phase 2: Generate React + Image Replacement
         → python3 scripts/search_images.py --dir frontend/src/pages
         → 验证: grep 无 placehold.co 残留
...
**⚠️ Mode 1 (6 Phase) 还原度要求最高，Mode 2/3 (5 Phase) 相对简化。**
...
> 🚨 Mode 3 必须先完成设计系统创建，跳过设计步骤会导致产品方向偏差！

1. **调用 `frontend-design` skill** 获取创意方向（必选，Phase 0）
2. **阅读 [mode3-design-workflow.md](references/mode3-design-workflow.md)** 的"PC/移动端高端感要点"
3. **确定"记忆点"**: 独特布局/字体处理/标志动画/大胆配色

```
Phase 0: Design Thinking (MANDATORY)
...
Phase 2: React Code Generation + Image Replacement (合并步骤)
...
|--------|------------------------------|----------------------------------|
| **Backend** | ❌ None | ✅ Express.js + TCB managed PostgreSQL |
