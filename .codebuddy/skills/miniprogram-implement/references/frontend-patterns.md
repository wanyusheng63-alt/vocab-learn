...
## Image Handling (CRITICAL - WeChat MiniProgram Limitation)

**NEVER use CSS `background-image: url(...)` with local file paths** — WeChat WXSS blocks local resource access. Use `<Image>` component instead:

```tsx
// ❌ WRONG — "本地资源图片无法通过 WXSS 获取"
...
`background-color` and `linear-gradient()` are fine — only `url()` with local paths is blocked.
...
11. **⚠️ Image handling**: No `background-image: url(...)` with local paths in SCSS — use `<Image>` component instead
