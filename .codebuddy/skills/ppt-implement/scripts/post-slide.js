...
function syncIndexHtmlWithSlides(indexHtmlPath, slidesDir, slideFiles) {
    try {
        ...
        // 防御并发写入导致的文件截断：
        // writeFileSync 内部先 O_TRUNC 截断文件为 0 字节再写入新内容，
        // 如果另一个进程恰好在 truncate 之后、write 之前读取，就会读到空文件。
        // 此时跳过写入，避免将空内容写回导致文件永久变空。
        if (!content || !content.includes('</html>')) {
            logger.warn('[syncIndexHtml] 读到异常内容（疑似并发 writeFileSync 导致的 O_TRUNC 截断，文件长度: ' + content.length + '），跳过本次写入');
            return;
        }
