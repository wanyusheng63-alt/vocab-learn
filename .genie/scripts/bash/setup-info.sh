...
APP_SLUG=""        # DNS-compliant app identifier for subdomain
...
show_help() {
    cat << EOF
Usage: setup_info.sh [--name PROJECT_NAME] [--app-slug APP_SLUG] [--preview-type PREVIEW_TYPE] [--project-type PROJECT_TYPE] VERSION_DESCRIPTION

Arguments:
  --name PROJECT_NAME           Project name (required for first run, optional afterwards)
  --app-slug APP_SLUG           DNS-compliant app identifier for subdomain (required for first run)
                                Must be 1-32 chars, lowercase letters/digits/hyphens, start and end with letter or digit
...
Examples:
  # First time setup (--name and --app-slug are required)
  setup_info.sh --name "My Project" --app-slug "my-project" "Initial version"
  setup_info.sh --name "My Project" --app-slug "my-project" --preview-type mobile --project-type web "Initial version"
  
  # Update version description (--name and --app-slug are optional, preserve existing values)
  setup_info.sh "Updated version with new features"
  setup_info.sh --preview-type mobile "Mobile-first version"
  setup_info.sh --name "My Project" --app-slug "my-project" --preview-type pc --project-type backend "Updated version"

Description:
  Creates or updates docs/project.json with the project name, app slug, version description, preview type, and project type.
  The --name and --app-slug parameters are required for the first run when project.json doesn't exist.
  For subsequent runs, they are optional and will preserve existing values if omitted.
  The --app-slug must be a valid DNS label: 1-32 lowercase alphanumeric characters and hyphens,
  must start and end with a letter or digit (e.g., "my-app", "cool-project-1").
...
parse_arguments() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --app-slug)
                if [ -z "$2" ] || [[ "$2" == --* ]]; then
                    echo "❌ Error: --app-slug requires a value"
                    echo ""
                    show_help
                    exit 1
                fi
...
read_existing_project_info() {
    if [ -f "$PROJECT_JSON" ]; then
        ...
        if command -v python3 &> /dev/null; then
            ...
            EXISTING_APP_SLUG=$(python3 -c "import json; print(json.load(open('$PROJECT_JSON')).get('app_slug', ''))" 2>/dev/null || echo "")
            ...
            EXISTING_APP_SLUG=$(grep -o '"app_slug"[[:space:]]*:[[:space:]]*"[^"]*"' "$PROJECT_JSON" | sed 's/"app_slug"[[:space:]]*:[[:space:]]*"\([^"]*\)"/\1/' || echo "")
        ...
        # 如果用户没有通过命令行指定 app_slug，使用现有值
        if [ -z "$APP_SLUG" ] && [ -n "$EXISTING_APP_SLUG" ]; then
            APP_SLUG="$EXISTING_APP_SLUG"
        fi
...
validate_app_slug() {
    local slug="$1"
    # 必须是 1-32 字符，小写字母/数字/连字符，首尾必须是字母或数字
    if ! echo "$slug" | grep -qE '^[a-z0-9]([a-z0-9-]{0,30}[a-z0-9])?$'; then
        echo "❌ Error: Invalid app_slug '$slug'"
        echo "   app_slug must be 1-32 characters, only lowercase letters (a-z), digits (0-9), and hyphens (-)"
        echo "   Must start and end with a letter or digit"
        echo "   Examples: 'my-app', 'cool-project-1', 'todo'"
        echo ""
        exit 1
    fi
}
...
validate_arguments() {
    ...
    # 如果是第一次运行，必须提供 --app-slug
    if [ ! -f "$PROJECT_JSON" ] && [ -z "$APP_SLUG" ]; then
        echo "❌ Error: --app-slug is required for first time setup"
        echo ""
        show_help
        exit 1
    fi
    ...
    # 确保有 app_slug
    if [ -z "$APP_SLUG" ]; then
        echo "❌ Error: Could not determine app_slug"
        echo ""
        show_help
        exit 1
    fi
