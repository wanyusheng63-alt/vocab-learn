...
*   **Chinese Text Word Wrap (CRITICAL)**: When creating text with `wordWrap`, you **MUST** add `useAdvancedWrap: true` to enable proper Chinese character line breaking. Without this, Chinese text will NOT wrap correctly.
    ```javascript
    // ❌ WRONG: Chinese text won't wrap properly
    this.add.text(x, y, '这是一段很长的中文文本...', {
        fontSize: '24px',
        wordWrap: { width: 300 }
    });

    // ✅ CORRECT: Enable advanced wrap for Chinese
    this.add.text(x, y, '这是一段很长的中文文本...', {
        fontSize: '24px',
...
**6.6. AI NPC DIALOGUE SYSTEM (When GDD Contains AI NPC Section)**

When `docs/product/features.md` or `docs/design/` contains an "AI NPC 对话系统" section, you MUST implement the complete AI dialogue system.

**Reference Document**: Read `.codebuddy/skills/game-2d-implement/references/ai-npc-dialogue.md` for complete implementation patterns.
...
2. **Create Backend Files** (copy patterns from reference):
   - `backend/src/lib/hunyuan-chat.ts` - SDK wrapper with sandbox proxy support
   - `backend/src/types/npc.types.ts` - Type definitions
   - `backend/src/modules/npc-dialogue.ts` - API endpoints (`/api/npc/dialogue`, `/api/npc/dialogue/stream`)
...
4. **Create Frontend Files**:
   - `game-basic/src/systems/DialogueManager.js` - Phaser dialogue UI system
   - `game-basic/src/data/npcs/*.json` - NPC configs from GDD specifications

5. **Integrate with Game**:
   - Initialize `DialogueManager` in game scene
   - Add NPC interaction triggers (collision/click/proximity)
   - Handle dialogue UI showing/hiding
...
### E3. AI NPC Dialogue System (If Required by GDD)

- [ ] **Detection**: Checked if `docs/product/features.md` contains "AI NPC 对话系统" section.
- [ ] **Backend SDK**: `tencentcloud-sdk-nodejs-hunyuan` is installed in backend.
- [ ] **SDK Wrapper**: `backend/src/lib/hunyuan-chat.ts` implements HunyuanChat class with sandbox proxy support.
- [ ] **API Endpoints**: `/api/npc/dialogue` (sync) and `/api/npc/dialogue/stream` (SSE) are implemented.
- [ ] **Route Registration**: `npcDialogueRouter` is registered in `app.ts`.
- [ ] **NPC Configs**: JSON files in `game-basic/src/data/npcs/` match GDD specifications.
- [ ] **DialogueManager**: Phaser UI system handles dialogue display, typing effect, and history.
- [ ] **Game Integration**: NPCs are interactive (proximity/click trigger) and dialogue UI works correctly.
...
### J. ⛔⛔⛔ Loading Page Replacement (CRITICAL - MUST FIX)

> **The template's default `Loading.js` is an infinite waiting animation with NO auto-navigation.**
> **If you don't fix this, the game will be stuck on the loading page forever!**

**MANDATORY Final Steps:**

1. **Create a real Preloader scene** (`src/scenes/Preloader.js`) that:
   - Loads all game assets with progress bar
   - **Auto-navigates to MainMenu** after loading completes:
     ```javascript
     this.load.on('complete', () => {
         this.scene.start('MainMenu');  // REQUIRED!
     });
     ```

2. **Update `main.js`** to replace Loading with Preloader:
   ```javascript
   // ❌ WRONG: Using infinite Loading scene
   scene: [Loading, TestPage]
   
   // ✅ CORRECT: Using Preloader with auto-navigation
   import { Preloader } from './scenes/Preloader.js';
   import { MainMenu } from './scenes/MainMenu.js';
   // ... other scenes
   
   scene: [Preloader, MainMenu, GamePlay, Settings, GameOver]
   ```

3. **Verify scene flow** by checking:
   ```bash
   # Must find scene.start calls in Preloader
   grep -r "scene.start" game-basic/src/scenes/Preloader.js
   
   # Must NOT have Loading as first scene (unless it's the real Preloader)
   grep "Loading" game-basic/src/main.js
   ```

**Self-Check before Done:**
- [ ] `main.js` imports a Preloader scene (not the template's infinite Loading)
- [ ] Preloader has `this.load.on('complete', () => this.scene.start('MainMenu'))`
- [ ] MainMenu has buttons that call `this.scene.start('GamePlay')` etc.
- [ ] Game can be played from start to finish without getting stuck