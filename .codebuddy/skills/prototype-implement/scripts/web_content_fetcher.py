...
# Optional: PIL for image compression (graceful fallback if not installed)
try:
    from PIL import Image
    HAS_PIL = True
except ImportError:
    HAS_PIL = False
    print("⚠️  PIL not installed, screenshots won't be compressed. Run: pip install Pillow", file=sys.stderr)


# Screenshot size limits to prevent 400 errors from AI models
MAX_SCREENSHOT_WIDTH = 1920  # Max width in pixels
MAX_SCREENSHOT_HEIGHT = 4000  # Max height to prevent huge long pages
MAX_SCREENSHOT_SIZE_MB = 4  # Target max file size in MB
JPEG_QUALITY = 85  # JPEG quality (0-100)


def compress_screenshot(image_path: Path, max_size_mb: float = MAX_SCREENSHOT_SIZE_MB) -> Path:
    """
    Compress screenshot to be under max_size_mb while maintaining reasonable quality.
    Converts PNG to JPEG for better compression on photos.
    ...
    Returns:
        Path to the compressed image (may be same as input if already small enough)
    """
    if not HAS_PIL:
        return image_path
    
    original_size = image_path.stat().st_size / (1024 * 1024)  # Size in MB
    
    if original_size <= max_size_mb:
        print(f"✅ Screenshot already small enough: {original_size:.2f}MB", file=sys.stderr)
        return image_path
    
    print(f"🗜️  Compressing screenshot: {original_size:.2f}MB → target <{max_size_mb}MB", file=sys.stderr)
    
    try:
        with Image.open(image_path) as img:
            # Get original dimensions
            orig_width, orig_height = img.size
            print(f"   Original size: {orig_width}x{orig_height}", file=sys.stderr)
            
            # Step 1: Resize if too large
            new_width, new_height = orig_width, orig_height
            
            # Limit width
            if new_width > MAX_SCREENSHOT_WIDTH:
                ratio = MAX_SCREENSHOT_WIDTH / new_width
                new_width = MAX_SCREENSHOT_WIDTH
                new_height = int(new_height * ratio)
            
            # Limit height (for very long pages)
            if new_height > MAX_SCREENSHOT_HEIGHT:
                new_height = MAX_SCREENSHOT_HEIGHT
                print(f"   ⚠️  Page truncated to {MAX_SCREENSHOT_HEIGHT}px height", file=sys.stderr)
            
            if (new_width, new_height) != (orig_width, orig_height):
                img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
                print(f"   Resized to: {new_width}x{new_height}", file=sys.stderr)
            
            # Step 2: Convert to RGB (JPEG doesn't support alpha)
            ...
            # Step 3: Save as JPEG with progressive compression
            ...
            for attempt in range(5):
                ...
                if size_mb <= max_size_mb:
                    ...
                    print(f"   ✅ Compressed to: {size_mb:.2f}MB (quality={quality})", file=sys.stderr)
                ...
                quality -= 10
                print(f"   Trying quality={quality}...", file=sys.stderr)
            
            # If still too large, save anyway with lowest quality tried
            with open(output_path, 'wb') as f:
                f.write(buffer.getvalue())
            print(f"   ⚠️  Best compression: {size_mb:.2f}MB", file=sys.stderr)
...
def fetch_web_content(url: str, output_dir: str):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 1. Capture full-page screenshot (with size limits)
            print("📸 Capturing screenshot...", file=sys.stderr)
            screenshot_path = output_path / "screenshot.png"
            
            # Get page height and limit if too long
            page_height = page.evaluate("() => document.documentElement.scrollHeight")
            if page_height > MAX_SCREENSHOT_HEIGHT:
                print(f"⚠️  Page very long ({page_height}px), capturing first {MAX_SCREENSHOT_HEIGHT}px only", file=sys.stderr)
                # Capture viewport-limited screenshot
                page.screenshot(
                    path=str(screenshot_path),
                    full_page=False,
                    clip={"x": 0, "y": 0, "width": 1920, "height": MAX_SCREENSHOT_HEIGHT}
                )
            else:
                page.screenshot(path=str(screenshot_path), full_page=True)
            
            # Compress screenshot to prevent 400 errors from AI models
            screenshot_path = compress_screenshot(screenshot_path)
            ...
            print(f"📸 screenshot.*: 视觉参考 (已压缩，<4MB)", file=sys.stderr)
            print("="*60 + "\n", file=sys.stderr)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  ├── screenshot.jpg    # 视觉参考 (已压缩)", file=sys.stderr)
